...
from gentlest_ascent_dynamics_on_manifolds import (
    IdealizedSaddleDynamics as ISD,
)
...
def test_isd_vectors():
    ...
    v = jnp.array(
        [
            eigenvectors[i, :, eigenvalue_permutation[i, 0]]
    ...
    isd_vectors1 = (
        ...
        * (
            jnp.einsum(
                '...i,...ij,...j', ascent_directions, g_points, grad_U_points
            )[..., None]
            * ascent_directions
        )
        - grad_U_points
    )
    isd_vectors1 /= jnp.sqrt(
        jnp.einsum('...i,...ij,...j', isd_vectors1, g_points, isd_vectors1)[
            ..., None
        ]
    )
