...
class DiffusionMaps:
    ...
    def __init__(
        self, num_eigenpairs: int, epsilon: Optional[float] = None
    ) -> None:
    ...
    def learn(self, points: jnp.ndarray) -> None:
        """Learn diffusion map coordinates for the given points."""
        distances2 = scipy.spatial.distance.pdist(points, metric='sqeuclidean')
        ...
        normalized_kernel_matrix = (
            (kernel_matrix * inv_sqrt_diag_vector).T * inv_sqrt_diag_vector
        ).T
